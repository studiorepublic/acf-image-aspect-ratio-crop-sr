...
/**
 * REST endpoint URLs for admin JS (uses rest_url for correct prefix/permalinks).
 *
 * @return array{upload: string, crop: string, get: string, preview: string}
 */
function aiarc_get_rest_endpoint_urls()
{
    return [
        'upload' => rest_url('aiarc/v1/upload'),
        'crop' => rest_url('aiarc/v1/crop'),
...
/**
 * Admin AJAX URL for crop preview images (avoids REST cookie auth on <img src>).
 *
 * @return string
 */
function aiarc_get_preview_ajax_base_url()
{
    return add_query_arg('action', 'aiarc_preview', admin_url('admin-ajax.php'));
}

/**
 * Generate cropped preview image bytes for admin display.
 *
 ...
 * @return array{mime: string, body: string}|WP_Error
 */
function aiarc_generate_preview_image($id, $x, $y, $w, $h)
{
    if ($id <= 0 || $w <= 0 || $h <= 0) {
        return new WP_Error(
...
/**
 * Send preview image HTTP response and exit.
 *
 * @param array{mime: string, body: string} $preview
 */
function aiarc_send_preview_image_response($preview)
{
    ...
    echo $preview['body']; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- binary image
    exit();
}
...
function aiarc_get_preview_url($crop_data)
{
    ...
    $url = add_query_arg(
        [
            ...
            'w' => (int) $c['width'],
            'h' => (int) $c['height'],
            '_wpnonce' => wp_create_nonce('aiarc_preview'),
        ],
...
/**
 * Whether the value is AIARC crop metadata (not a standard ACF image array).
 *
 * @param mixed $value Field value or image data.
 * @return bool
 */
function aiarc_is_crop_data($value)
{
    return is_array($value) &&
        !empty($value['attachment_id']) &&
        !empty($value['crop']) &&
        isset(
            $value['crop']['x'],
            $value['crop']['y'],
...
/**
 * Resolve a WordPress attachment ID from common image value shapes.
 *
 * @param mixed $image Attachment ID, ACF image array, Timber image, or crop metadata.
 * @return int Attachment ID or 0.
 */
function aiarc_resolve_attachment_id($image)
{
    if (is_numeric($image)) {
        $id = (int) $image;
...
/**
 * Resolve an image URL from common value shapes (no cropping).
 *
 * @param mixed $image Attachment ID, URL string, ACF image array, or Timber image.
 * @return string
 */
function aiarc_resolve_image_url($image)
{
    if (is_string($image)) {
        return $image;
    }

    $attachment_id = aiarc_resolve_attachment_id($image);
    if ($attachment_id) {
...
/**
 * Cloudflare transform URL to scale an image (no trim).
 *
 * @param string $source Absolute image URL.
 * @param int    $max_width  Optional max width.
 * @param int    $max_height Optional max height.
 * @return string Transform path or empty string.
 */
function aiarc_cloudflare_resize_url($source, $max_width = 0, $max_height = 0)
{
    if (empty($source)) {
        return '';
    }
...
/**
 * URL for a standard (non-cropped) image, optionally scaled.
 *
 * @param mixed $image Attachment ID, URL, ACF image array, or Timber image.
 * @param int   $max_width  Optional max width.
 * @param int   $max_height Optional max height.
 * @return string
 */
function aiarc_standard_image_url($image, $max_width = 0, $max_height = 0)
{
    $url = aiarc_resolve_image_url($image);
    if ($url === '') {
        return '';
    }
    ...
    if ($cloudflare_images && aiarc_is_cloudflare_proxy()) {
        if ($max_width > 0 || $max_height > 0) {
            $cf_url = aiarc_cloudflare_resize_url($url, $max_width, $max_height);
            if ($cf_url !== '') {
                return $cf_url;
            }
        }
    ...
    $cache_key = 'aiarc_full_' . md5($attachment_id . $max_width . $max_height);
...
/**
 * Generate image URL from crop metadata or a standard image field value.
 *
 * Accepts AIARC crop metadata, ACF image arrays, attachment IDs, image URLs,
 * or Timber image objects. Non-cropped values return the image URL (scaled when
 * max dimensions are passed).
 *
...
function aiarc_crop_url($image, $max_width = 0, $max_height = 0)
{
    ...
    if (!aiarc_is_crop_data($image)) {
        return aiarc_standard_image_url($image, $max_width, $max_height);
    }
...
class npx_acf_plugin_image_aspect_ratio_crop
{
    ...
    function __construct()
    {
        ...
        add_action('wp_ajax_aiarc_preview', [$this, 'ajax_preview_handler']);
    ...
    /**
     * Admin AJAX: serve cropped preview for <img src> (no REST cookie auth).
     */
    public function ajax_preview_handler()
    {
        ...
        aiarc_debug_log(
            'acf-image-aspect-ratio-crop-sr.php:ajax_preview_handler',
            ...
            [
                ...
                'nonce_ok' => isset($_GET['_wpnonce'])
                    && wp_verify_nonce(sanitize_text_field(wp_unslash($_GET['_wpnonce'])), 'aiarc_preview'),
            ],
            'H2'
        );
        ...
        check_ajax_referer('aiarc_preview', '_wpnonce');

        $preview = aiarc_generate_preview_image($id, $x, $y, $w, $h);
        if (is_wp_error($preview)) {
            $status = $preview->get_error_data()['status'] ?? 500;
            aiarc_debug_log(
                'acf-image-aspect-ratio-crop-sr.php:ajax_preview_handler',
                'ajax preview error',
                ['code' => $preview->get_error_code(), 'status' => $status],
                'H5'
            );
        ...
        aiarc_debug_log(
            'acf-image-aspect-ratio-crop-sr.php:ajax_preview_handler',
            'ajax preview success',
            ['mime' => $preview['mime'], 'bytes' => strlen($preview['body'])],
            'H5'
        );

        aiarc_send_preview_image_response($preview);
    }
    ...
    public function rest_api_preview_callback(WP_REST_Request $request)
    {
        ...
        aiarc_debug_log(
            'acf-image-aspect-ratio-crop-sr.php:rest_api_preview_callback',
            'rest preview request',
            ['id' => $id, 'w' => $w, 'h' => $h, 'logged_in' => is_user_logged_in()],
            'H2'
        );
        ...
        $response = new WP_REST_Response($preview['body'], 200);
        $response->header('Content-Type', $preview['mime']);
